#!/bin/bash
# for the experiment in ATR 2FW with two posters (since 2024.03.07)
session="telecoV"
...
tmux split-window -v
tmux split-window -v
tmux send-keys -t $session:$base_window_name.0 "roslaunch telecoV navigation_teb_diff.launch rover_type:=mega3 start_rviz:=false map_file:=atr_2fpath.yaml" Enter
sleep 5
