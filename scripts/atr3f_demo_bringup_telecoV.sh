...
waypoint_file="waypoints.bin.atr3f"
tmux new-session -d -s $session -n $base_window_name;
tmux split-window -v
tmux split-window -h
tmux split-window -v
tmux split-window -v
tmux send-keys -t $session:$base_window_name.0 "roslaunch telecoV navigation_teb_diff.launch start_rviz:=false map_file:=atr_3f.yaml" Enter
sleep 5
...
#tmux split-window -v
tmux split-window -h
tmux send-keys -t $session:$capf_window_name.0 "roslaunch takasaki_capf no_stt_teleco_basic.launch stress_test:=false" Enter
