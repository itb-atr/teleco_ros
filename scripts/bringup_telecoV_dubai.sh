...
tmux split-window -v
tmux split-window -v
tmux send-keys -t $session:$base_window_name.0 "roslaunch telecoV navigation_teb_diff.launch rover_type:=mega3 start_rviz:=false map_file:=dubai_dfl_1f.yaml" Enter
sleep 5
...
tmux send-keys -t $session:$capf_window_name.1 "rviz -d ~/catkin_ws/src/telecoV/rviz/sim.rviz" Enter
